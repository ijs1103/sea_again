...
import { getWater } from '@utils/openApi'
import { cls } from '@utils/index'
import { useQuery } from '@tanstack/react-query'
import Loader from '@components/Loader'
import { WaterResponse } from '@utils/interfaces'
...
function WaterQuality({ sido_nm, sta_nm }: Props) {
	const { data, isLoading } = useQuery<WaterResponse>(['water'], () => getWater(sido_nm, sta_nm), { enabled: !!sido_nm && !!sta_nm })
	console.log(data)
	return (
		<div className='h-full flex flex-col justify-center items-center relative'>
			{isLoading ? <Loader /> :
				<>
					...
					<span className={cls('mt-3 text-2xl ', data?.ok ? 'text-safe' : 'text-danger')}>{data?.ok ? '적합' : '부적합'}</span>
