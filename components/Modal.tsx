				...
				<div className="w-[340px] sm:w-full px-4 py-2 rounded-lg">
					<div className='flex flex-col items-center gap-3'>
						...
						<div className='flex items-center justify-between w-full'>
							<RWebShare data={{ text: `${beachData?.sido_nm} ${beachData?.gugun_nm}에 위치 하고 있습니다`, url: beachData?.link_addr, title: `${beachData?.sta_nm} 해수욕장` }} onClick={() => console.log('공유 하기 버튼 클릭')}>
							...
							<div className='text-center'>
								<h3 className='text-lg font-bold sm:text-xl text-fontPrimary'>{beachData?.sta_nm}</h3>
								<span className='text-xs text-fontSecondary'>{beachData?.sido_nm} {beachData?.gugun_nm}</span>
							</div>
						...
						<ul className='w-[400px] flex justify-center text-center'>
							<li className='flex-1'>
								...
								<span className={cls('text-xs ', beachData?.beach_knd ? 'text-blue-500' : 'text-fontSecondary')}>{beachData?.beach_knd ? beachData?.beach_knd : '없음'}</span>
							</li>
							<li className='flex-1'>
								<span className='modal-info'>연락처</span>
								<span className={cls('text-xs ', beachData?.link_tel ? 'text-blue-500' : 'text-fontSecondary')}>{beachData?.link_tel ? extractOnlyPhoneNum(beachData?.link_tel) : '없음'}</span>
							</li>
							<li className='flex-1'>
								<span className='modal-info'>홈페이지</span>
								<a onClick={e => !beachData?.link_addr && e.preventDefault()} href={beachData?.link_addr} target="_blank" rel="noopener noreferrer">
									<span className={cls('text-xs ', beachData?.link_addr ? 'text-blue-500 hover:underline' : 'text-fontSecondary')}>
										{beachData?.link_addr ? '링크' : '없음'}
									</span>
								</a>
							</li>
						</ul>
