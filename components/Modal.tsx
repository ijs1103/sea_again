...
const Modal = (props: BeachResponse) => {
	const { beach_img, sido_nm, gugun_nm, sta_nm, link_addr, link_tel, beach_knd } = props
		...
		<div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg shadow-xl bg-white">
			<div className="w-full rounded-lg px-4 py-2">
				<div className='flex flex-col items-center gap-3'>
					{beach_img ? <Image src={beach_img} width={500} height={130} alt={beach_img} /> : <div className='bg-gray w-[500px] h-[130px] rounded-lg'></div>}
					<div className='w-full flex justify-between items-center'>
						<svg className='w-8 h-8 fill-modalPrimary' viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
					...
					<ul className='w-[400px] flex justify-center gap-3 text-center'>
						<li>
							<span className='block text-fontPrimary font-bold'>홈페이지?</span>
							<a onClick={e => !link_addr && e.preventDefault()} href={link_addr}>
								<span className={cls('text-xs ', link_addr ? 'text-blue-500 hover:underline' : 'text-fontSecondary')}>
									{link_addr ? '링크' : 'Not Found'}
								</span>
							</a>
						</li>
						<li>
							<span className='block text-fontPrimary font-bold'>연락처?</span>
							<span className='text-fontSecondary text-xs'>{link_tel ? link_tel : 'Not Found'}</span>
						</li>
						<li>
							<span className='block text-fontPrimary font-bold'>특징?</span>
							<span className='text-fontSecondary text-xs'>{beach_knd ? beach_knd : 'Not Found'}</span>
						</li>
					</ul>
					<div className='w-full bg-modalPrimary rounded-3xl'>
						<ul className='flex justify-center px-6 py-2'>
							<li className={cls('flex-1 py-1 text-center rounded-xl text-[12px] ', isCurrent ? 'bg-white text-fontPrimary shadow-2xl' : 'bg-transparent text-white')}>
								<span>날씨</span>
							</li>
							<li className={cls('flex-1 py-1 text-center rounded-xl text-[12px] ', isCurrent ? 'bg-white text-fontPrimary shadow-2xl' : 'bg-transparent text-white')}>
								<span>날씨</span>
					...
					<div className='mt-4 mx-4 bg-gray'>
